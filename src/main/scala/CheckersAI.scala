...
object CheckersAI {
  ...
  def minimax(
    ...
    if (isMaximizing) {
      ...
      for (move <- possibleMoves) {
        ...
        val eval =
          minimax(newBoard, depth - 1, false, a, b, maximizingPlayerIsBlack)
      ...
      for (move <- possibleMoves) {
        val newBoard = applyMove(board, move)
        val eval =
          minimax(newBoard, depth - 1, true, a, b, maximizingPlayerIsBlack)
  ...
  def iterativeDeepening(
    ...
    while (depth <= maxDepth && !timeLimitExceeded) {
      val elapsedMillis = System.currentTimeMillis() - aiStartTime
      ...
      if (tempBestMove.isEmpty) {
        println(
          s"Time limit exceeded before completing depth $depth. Returning best move from depth ${depth - 1}."
        )
  ...
  def bestMoveAtDepth(
    ...
    for (move <- moves) {
      ...
      if (currentTime - startTime > timeLimitMillis) {
        println("Time limit exceeded during move evaluation.")
        boundary.break(bestMoveFound)
      }
    ...
    if (bestMoveFound.isDefined) {
      println(
        f"Best move at depth $depth: ${bestMoveFound.get} with eval: $bestEval%.2f"
      )
    }
