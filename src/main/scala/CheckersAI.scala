...
object CheckersAI {
  ...
  def minimax(board: Board,
              ...
              alpha: Double,
              beta: Double,
              maximizingPlayerIsBlack: Boolean): Double = 
    parallelMinimax(board, depth, isMaximizing, alpha, beta, maximizingPlayerIsBlack, false)

  def parallelMinimax(board: Board,
                    depth: Int,
                    isMaximizing: Boolean,
                    alpha: Double,
                    beta: Double,
                    maximizingPlayerIsBlack: Boolean,
                    forceSequential: Boolean = false): Double = boundary {
    if (depth == 0 || isGameOver(board)) {
      return evaluateBoard(board, maximizingPlayerIsBlack)
    }
    
    // Current player's turn in this board state
    val currentPlayerIsBlack = if (isMaximizing) maximizingPlayerIsBlack else !maximizingPlayerIsBlack
    ...
    if (!forceSequential && depth >= PARALLEL_THRESHOLD && possibleMoves.size > 1) {
      if (isMaximizing) {
        ...
        var maxEval = parallelMinimax(firstBoard, depth - 1, false, currentAlpha.get(), beta, maximizingPlayerIsBlack, true)
        ...
        val futureEvals = remainingMoves.map { move =>
          Future {
            ...
            parallelMinimax(newBoard, depth - 1, false, currentAlpha.get(), beta, maximizingPlayerIsBlack, true)
          }
        }
        ...
        var minEval = parallelMinimax(firstBoard, depth - 1, true, alpha, currentBeta.get(), maximizingPlayerIsBlack, true)
        ...
        val futureEvals = remainingMoves.map { move =>
          Future {
            val newBoard = applyMove(board, move)
            parallelMinimax(newBoard, depth - 1, true, alpha, currentBeta.get(), maximizingPlayerIsBlack, true)
          }
        }
      ...
      if (isMaximizing) {
        ...
        for (move <- possibleMoves) {
          val newBoard = applyMove(board, move)
          val eval = parallelMinimax(newBoard, depth - 1, false, a, b, maximizingPlayerIsBlack, true)
        ...
        for (move <- possibleMoves) {
          val newBoard = applyMove(board, move)
          val eval = parallelMinimax(newBoard, depth - 1, true, a, b, maximizingPlayerIsBlack, true)
  ...
  def evaluateBoard(board: Board, isBlackTurn: Boolean): Double = {
    ...
    def positionalValue(piece: Piece, row: Int, col: Int): Double = {
      ...
      val centerBonus = if (isInnerCenter(row, col)) 0.3 
      ...
      val kingMobility = if (piece.isKing) countPieceMoves(board, row, col) * mobilityFactor else 0.0
      
      // Formation bonuses
      val formationBonus = if (hasFriendlyNeighbor(piece, row, col)) 0.1 else -0.1
      
      // Safety bonus
      val safetyBonus = if (isSafe(piece, row, col)) 0.2 else 0.0
      
      // Back row bonus (defensive position)
      val backRowBonus = if ((isBlack && row == 0) || (!isBlack && row == 7)) 0.12 else 0.0
  ...
  def bestMoveAtDepth(board: Board, isBlackTurn: Boolean, depth: Int, timeLimitMillis: Long): Option[Move] = {
    ...
    val futures = moves.map { move =>
      Future {
        ...
        val eval = parallelMinimax(newBoard, depth - 1, false, Double.NegativeInfinity, Double.PositiveInfinity, isBlackTurn)
        (move, eval)
      }
    }
